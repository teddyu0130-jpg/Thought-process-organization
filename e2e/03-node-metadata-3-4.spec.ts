...
test.describe('3-4. ステータス変更に応じてノードの見た目が変化する', () => {
  ...
  test('ノードAを選定済み、ノードBを却下にすると見た目が変わり、テーマ一覧のバッジも反映される', async ({ page }) => {
    ...
    await childNodes.nth(1).getByTestId('node-click-area').dispatchEvent('click')
    ...
    await expect(page.getByText('選定済み').or(page.getByText('検討中')).or(page.getByText('却下')).first()).toBeVisible()
  })
})
